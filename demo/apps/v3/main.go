// Demo app v3 — major bump over v1/v2 for the OTA updater demo.
//
// What's new vs v2:
//   - Version bumped 1.1.0 → 2.0.0 (semver major).
//   - New endpoint /status returns a small JSON document.
//   - Banner page background flips green → red.
//   - /hello from v1.1.0 still works.
package main
...
const (
	version        = "2.0.0"
	color          = "#7f1e2e" // red
	title          = "Major bump — /status endpoint added"
...
<body>
  <h1>v{{.Version}}</h1>
  <p>{{.Title}}</p>
  <p><span class="new">NEW</span> <code>GET /status</code> · /hello still here</p>
...
func main() {
	...
	mux.HandleFunc("/status", handleStatus) // new in v2.0.0
...
func handleHello(w http.ResponseWriter, _ *http.Request) {
	...
	fmt.Fprintf(w, "Hello from v%s! Endpoint introduced in v1.1.0.\n", version)
}
